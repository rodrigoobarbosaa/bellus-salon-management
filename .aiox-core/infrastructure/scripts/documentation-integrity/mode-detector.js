 ...
 * 1. FRAMEWORK_DEV - .aiox-core/ exists AND is aios-core repo
...
function detectInstallationMode(targetDir = process.cwd()) {
  ...
  if (markers.hasAiosCore && markers.isAiosCoreRepo) {
    return {
      ...
      confidence: 100,
      reason: 'Detected aios-core repository with .aiox-core directory',
      markers,
    };
  }
...
function collectMarkers(targetDir) {
  ...
  return {
    // AIOX markers
    hasAiosCore: fs.existsSync(path.join(targetDir, '.aiox-core')),
...
function isAiosCoreRepository(targetDir) {
  ...
  try {
    ...
    const hasAiosMarker = fs.existsSync(path.join(targetDir, '.aiox-core', 'infrastructure'));
    const hasWorkspaces =
...
function mapLegacyTypeToMode(legacyType, context = {}) {
  // EXISTING_AIOX is ambiguous: it can be FRAMEWORK_DEV (aios-core repo)
